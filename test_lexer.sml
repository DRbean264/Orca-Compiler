structure Test =
struct 
  fun test {filename, test_tokens, line_num} =
      ...
      in
        ...
        if token_list <> test_tokens then 
          print ("\027[91mTest on " ^ filename ^ " failed!\nExpected tokens: \n[" ^ (tl_to_string test_tokens) ^ "]\n, got \n["
            ^ (tl_to_string token_list) ^ "]\027[m\n")
        else if line_num <> !ErrorMsg.lineNum then
            print ("\027[91mTest on " ^ filename ^ " failed!\nExpected line num: " ^ (Int.toString line_num) ^ ", but got: "
              ^ (Int.toString (!ErrorMsg.lineNum)) ^ " instead\027[m\n") else print ("\027[92mTest on " ^ filename ^ " passed!\027[m\n");
        token_list = test_tokens andalso line_num = !ErrorMsg.lineNum
      end
  ...
  val test_answers = 
    [
      {
        filename = "testcases/teststrings.tig",
        test_tokens = [
          Tokens.LET (25, 0),
          Tokens.VAR (33, 0),
          Tokens.ID (("s1"), 37, 0),
